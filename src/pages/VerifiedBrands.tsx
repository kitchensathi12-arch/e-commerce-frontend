      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {data?.map((brand: IBrandDocument) => (
          <div key={brand._id.toString()} className="bg-white rounded-2xl shadow-md hover:shadow-xl transition duration-300 p-5 flex flex-col items-center text-center">
            ...
            {/* Description */}
            <p className="text-gray-500 text-sm mt-2">{brand.description}</p>

            {/* Authorized Badge */}
            {brand.authorized && <span className="mt-3 px-3 py-1 text-xs bg-green-100 text-green-600 rounded-full">Authorized ✔</span>}

            {/* Certificate */}
            <a href={brand.certificate} target="_blank" rel="noopener noreferrer" className="mt-4 text-blue-500 text-sm underline">
              View Certificate
            </a>
          </div>
        ))}
