...
export default function ProductDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-stone-50 font-sans text-gray-900">
      {/* Breadcrumb */}
      <div className="max-w-screen-xl mx-auto px-6 lg:px-10 py-3.5"></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 xl:gap-14">
          ...
          <div className="flex gap-3 lg:gap-4">
              ...
              {productDetails?.product_images?.map((img) => (
                <button
                  ...
                  className={`w-16 h-16 rounded-xl overflow-hidden transition-all duration-200 ${
                    activeImg === img?.image_url ? 'ring-2 ring-zinc-900 ring-offset-1' : 'ring-1 ring-stone-200 hover:ring-zinc-400 opacity-60 hover:opacity-100'
                  }`}
                >
            ...
            <div className="relative flex-1 rounded-2xl overflow-hidden bg-stone-100 group cursor-zoom-in" style={{ minHeight: 560 }}>
              <img src={activeImg} alt="Product" className="w-full object-cover transition-opacity duration-300" style={{ minHeight: 560, maxHeight: 700 }} />
          ...
          <div className="flex flex-col gap-5 lg:pt-2">
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <a href="#" className="text-xs text-zinc-400 hover:text-zinc-700 underline underline-offset-2 transition-colors">
                312 reviews
              </a>
              <span className="text-stone-300 text-sm">|</span>
            ...
            <div className="flex items-center gap-3 bg-stone-100 rounded-2xl px-5 py-4">
              ...
              <div className="ml-auto text-center">
                <span className="bg-green-600 text-white text-xs font-bold px-2.5 py-1 rounded-lg block">
                  SAVE ₹{productDetails?.product_mrp_price && productDetails?.product_selling_price ? (productDetails.product_mrp_price - productDetails.product_selling_price).toFixed(2) : '0.00'}
                </span>
            ...
            <div>
              ...
              <p className="text-sm text-zinc-600 leading-relaxed">
                ...
                {showMore &&
                  ' The fabric has a natural slubbed texture for an effortlessly undone look. Machine washable for everyday ease. Cut in a generous, oversized silhouette for that coveted borrowed-from-the-boys aesthetic. 100% responsibly sourced cotton blend.'}
              </p>
              <h3>Additional details</h3>
              <div>
                {productDetails?.product_details?.map((detail: any) => (
            ...
            <div className="flex gap-3 items-stretch">
              <div className="flex items-center rounded-xl border-2 border-stone-200 overflow-hidden bg-white">
                <button
                  ...
                  className="w-11 h-full flex items-center justify-center text-zinc-400 hover:bg-stone-100 transition-colors text-xl font-light select-none"
                >
                  −
                </button>
                <span className="w-10 text-center text-sm font-bold text-zinc-800 select-none">{qty}</span>
                <button onClick={() => setQty((q) => q + 1)} className="w-11 h-full flex items-center justify-center text-zinc-400 hover:bg-stone-100 transition-colors text-xl font-light select-none">
                  +
                </button>
              </div>
              ...
              <button
                onClick={handleAdd}
                className={`flex-1 py-3.5 rounded-xl text-sm font-bold tracking-wide transition-all duration-300 ${
                  added ? 'bg-emerald-600 text-white' : 'bg-zinc-900 text-white hover:bg-zinc-700 active:scale-95'
                }`}
              >
