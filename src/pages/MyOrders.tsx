...
import type { IOrderDocument, OrderStatus, PaymentStatus } from '@kitchensathi12-arch/ecommerce-types';
...
const MOCK_ORDERS: IOrderDocument[] = [
  ...
  } as unknown as IOrderDocument,
  {
    ...
    items: [{ product: new Types.ObjectId(), name: 'Pressure Cooker 5L Aluminium', price: 1850, quantity: 1, image: '⚙️' }],
  } as unknown as IOrderDocument,
  {
    _id: new Types.ObjectId(),
    user: new Types.ObjectId(),
  ...
  } as unknown as IOrderDocument,
  {
    ...
    items: [{ product: new Types.ObjectId(), name: 'Electric Kettle 1.5L SS Body', price: 999, quantity: 1, image: '☕' }],
  } as unknown as IOrderDocument,
];

// ─── Display configs keyed by your exact enum values ─────────────────────────

const STATUS_CONFIG: Record<OrderStatus, { label: string; color: string; bg: string; dot: string; trackStep: number }> = {
...
function TrackingBar({ orderStatus }: { orderStatus: OrderStatus }) {
  ...
  const steps: { label: string }[] = [{ label: 'Ordered' }, { label: 'Confirmed' }, { label: 'Shipped' }, { label: 'Delivered' }];
    ...
    >
      <div
        style={{
          ...
          alignItems: 'center',
          justifyContent: 'center',
          fontSize: isEmoji ? 26 : 12,
          flexShrink: 0,
          overflow: 'hidden',
        }}
      >
        {item.image ? isEmoji ? item.image : <img src={item.image} alt={item.name} style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 8 }} /> : '📦'}
      </div>
      <div style={{ flex: 1 }}>
      ...
      <div style={{ textAlign: 'right' }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#e07b54' }}>₹{(item.price * item.quantity).toLocaleString('en-IN')}</div>
                    ...
                    <div style={{ padding: '20px 22px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 24, marginTop: 20, flexWrap: 'wrap' }}>
                        ...
                        <div style={{ flex: 2, minWidth: 280 }}>
                          <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: '#9ca3af', marginBottom: 12 }}>Items Ordered</div>
                        ...
                        <div style={{ flex: 1, minWidth: 220, display: 'flex', flexDirection: 'column', gap: 16 }}>
                          ...
                          <div style={{ background: '#1a1a1a', borderRadius: 12, padding: '16px 18px', color: '#fff' }}>
                            <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: '#9ca3af', marginBottom: 12 }}>Order Summary</div>
                            {[
                          ...
                          >
                            <div style={{ fontWeight: 700, fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, color: '#9ca3af', marginBottom: 4 }}>Delivery Details</div>
                            <div>💳 {order.paymentMethod}</div>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                              <span>Payment Status</span>
                              <span style={{ fontWeight: 700, color: pc.color }}>{pc.label}</span>
                            </div>
                            {order.isDelivered && order.deliveredAt && <div>✅ Delivered on {fmtDate(order.deliveredAt)}</div>}
                            {order.paidAt && <div>💰 Paid on {fmtDate(order.paidAt)}</div>}
                          </div>
