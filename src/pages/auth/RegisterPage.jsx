...
const MobileBanner = () => (
  <div className="lg:hidden w-full px-6 pt-10 pb-8 text-center" style={{ background: 'linear-gradient(160deg, #7a4a22 0%, #3e2610 100%)' }}>
    ...
    <p className="text-[13px]" style={{ color: 'rgba(255,255,255,0.55)' }}>
      Join KitchenSathi and unlock exclusive deals &amp; more.
    </p>
  </div>
);
...
const RegisterPage = () => {
  ...
  const handleSubmit = async () => {
    const e = RegisterValidation(form);
      ...
      >
        ...
        <div className="relative flex-col justify-between p-12 overflow-hidden hidden lg:flex" style={{ background: 'linear-gradient(145deg, #7a4a22 0%, #5c3a1e 50%, #3e2610 100%)' }}>
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 gap-3">
              {FEATURES.map(({ icon, label }) => (
                <div key={label} className="flex items-center gap-2 text-[13px] rounded-2xl px-4 py-3 transition-colors hover:bg-white/[0.12]" style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.10)', color: 'rgba(255,255,255,0.80)' }}>
          ...
          <div className="relative z-10 flex gap-2 mt-8">
            <button onClick={() => navigate('/login')} className="h-2 w-2 rounded-full transition-all hover:bg-white/50" style={{ background: 'rgba(255,255,255,0.25)' }} />
