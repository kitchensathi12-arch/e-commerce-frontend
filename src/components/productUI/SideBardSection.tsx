          ...
          /> */}
            {options?.map((option) => (
              <label key={option.id} className="flex items-center gap-2.5 cursor-pointer group">
                <input type="checkbox" checked={selected.includes(option.id)} onChange={() => toggle(option.id)} className="w-4 h-4 rounded accent-purple-600 cursor-pointer" />
                <span className="text-sm text-gray-700 flex-1 group-hover:text-gray-900">{option.name}</span>
                <span className="text-xs text-gray-400">({option.count})</span>
              </label>
            ))}
            {options?.length === 0 && <p className="text-xs text-gray-400 py-1">No results found</p>}
          </div>
        )}
      </div>
    </>
