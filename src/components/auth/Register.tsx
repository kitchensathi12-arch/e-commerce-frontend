    ...
    <div className="relative min-h-screen bg-[url('/Bg.jpg')] bg-cover bg-bottom flex items-center justify-center px-4 py-10">
      ...
      <div className="relative w-full max-w-5xl rounded-2xl shadow-2xl grid grid-cols-1 md:grid-cols-2 overflow-hidden">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="mt-3 text-sm leading-relaxed" style={{ color: '#c9b8a0' }}>
              Join us and start managing<br />everything in one place.
            </p>
          </div>
        ...
        <div className="p-6 sm:p-8 lg:p-10" style={{ background: '#faf7f2' }}>
          <h1 className="text-2xl font-bold" style={{ color: '#2a1508', fontFamily: 'Georgia, serif' }}>Get started</h1>
          <p className="mt-1 text-sm" style={{ color: '#8a7060' }}>Create your account in just a few steps</p>
          ...
          <form onSubmit={formik.handleSubmit} className="mt-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {/* Full Name */}
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#4a2e1a' }}>Full name</label>
                <input
                  name="full_name"
                  type="text"
                ...
                {formik.touched.full_name && formik.errors.full_name && (
                  <p className="mt-1 text-xs" style={{ color: '#e03e2d' }}>{formik.errors.full_name}</p>
                )}
              </div>

              {/* Email */}
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#4a2e1a' }}>Email address</label>
                <input
                  name="email"
                  type="email"
                ...
                {formik.touched.email && formik.errors.email && (
                  <p className="mt-1 text-xs" style={{ color: '#e03e2d' }}>{formik.errors.email}</p>
                )}
              </div>

              {/* Phone */}
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#4a2e1a' }}>Phone number</label>
                <input
                  name="phone"
                  type="text"
                ...
                {formik.touched.phone && formik.errors.phone && (
                  <p className="mt-1 text-xs" style={{ color: '#e03e2d' }}>{formik.errors.phone}</p>
                )}
              </div>

              {/* Username */}
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#4a2e1a' }}>Username</label>
                <input
                  name="username"
                  type="text"
            ...
            {/* Password */}
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: '#4a2e1a' }}>Password</label>
              <input
                name="password"
