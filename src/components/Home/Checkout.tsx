    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <nav className="text-xs sm:text-sm text-gray-400 flex flex-wrap items-center gap-1 sm:gap-2">
          <span className="hover:text-gray-600 cursor-pointer" onClick={() => navigate('/')}>
            Account
          </span>
          <span>/</span>
          ...
          <span>/</span>
          <span className="hover:text-gray-600 cursor-pointer" onClick={() => navigate('/cart')}>
            View Cart
          </span>
          <span>/</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-20">
        <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900 mb-8 sm:mb-10">Billing Details</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex items-center gap-3 mt-1">
              <input type="checkbox" id="saveInfo" name="saveInfo" checked={form.saveInfo} onChange={handleChange} className="w-4 h-4 accent-red-500 cursor-pointer flex-shrink-0" />
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-5">
              {cartItems.map((item) => (
                <div key={item.id} className="flex items-center justify-between gap-4">
                  ...
                  <span className="text-sm font-medium text-gray-900 whitespace-nowrap">${item.price.toLocaleString()}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="radio" name="payment" value="bank" checked={paymentMethod === 'bank'} onChange={() => setPaymentMethod('bank')} className="accent-red-500 w-4 h-4" />
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="radio" name="payment" value="cod" checked={paymentMethod === 'cod'} onChange={() => setPaymentMethod('cod')} className="accent-red-500 w-4 h-4" />
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              />
              <button className="bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-md text-sm font-medium transition-colors whitespace-nowrap">Apply Coupon</button>
            </div>

            {/* Place Order */}
            <button onClick={handlePlaceOrder} className="w-full sm:w-48 bg-red-500 hover:bg-red-600 text-white py-3 rounded-md font-medium text-sm transition-colors">
              Place Order
            </button>
          </div>
        </div>
