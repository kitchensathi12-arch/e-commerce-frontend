...
const inp: React.CSSProperties = {
  ...
  border: "1px solid #e0d9d1", borderRadius: 8,
  fontSize: 13.5, fontFamily: "inherit",
  color: "#2b2420", background: "#faf8f5", outline: "none",
};
    ...
    <div>
      ...
      <div style={{ position: "relative" }}>
        ...
        <button
          ...
          style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#b0a89f", display: "flex", alignItems: "center" }}
        >
...
const AccountPage: React.FC = () => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f0eb", padding: "2rem 1rem 3rem", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #e8e2da", overflow: "hidden", boxShadow: "0 2px 20px rgba(0,0,0,0.06)", marginBottom: "1.25rem" }}>
          ...
          <div style={{ background: "linear-gradient(135deg, #c0392b 0%, #e74c3c 100%)", padding: "1.5rem 2rem", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, position: "relative", zIndex: 1 }}>
              <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(255,255,255,0.22)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 600, color: "#fff", border: "2px solid rgba(255,255,255,0.4)", flexShrink: 0 }}>
                {info.firstName[0]}{info.lastName[0]}
              </div>
              <div>
                <div style={{ fontSize: 18, fontWeight: 600, color: "#fff" }}>{info.firstName} {info.lastName}</div>
          ...
          <div style={{ display: "flex", gap: 0, padding: "0 1.5rem" }}>
            {(["profile", "orders"] as Tab[]).map((t) => (
              <button
                ...
                style={{ padding: "0.85rem 1.25rem", fontSize: 13.5, fontWeight: tab === t ? 600 : 400, color: tab === t ? "#e74c3c" : "#9e9085", background: "none", border: "none", borderBottom: tab === t ? "2px solid #e74c3c" : "2px solid transparent", cursor: "pointer", textTransform: "capitalize", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6, transition: "color 0.15s" }}
              >
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1.25rem 2rem", borderBottom: "1px solid #f0ebe4" }}>
              ...
              {!editing && (
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 6, height: 32, padding: "0 14px", borderRadius: 8, border: "1px solid #ddd6cc", background: "transparent", fontSize: 13, fontFamily: "inherit", color: "#6b6360", cursor: "pointer" }}
                >
            ...
            <div style={{ padding: "1.5rem 2rem" }}>
              {!editing ? (
                ...
                <div>
                  ...
                  <div style={{ padding: "1rem 1.25rem", background: "#faf8f5", borderRadius: 10, border: "1px solid #ede8e1", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
                    <button onClick={handleCancel} style={{ height: 34, padding: "0 16px", borderRadius: 8, border: "1px solid #ddd6cc", background: "transparent", fontSize: 13.5, fontFamily: "inherit", color: "#6b6360", cursor: "pointer" }}>
                      Cancel
                    </button>
                    <button onClick={handleSave} style={{ height: 34, padding: "0 20px", borderRadius: 8, border: "none", background: "linear-gradient(135deg,#c0392b,#e74c3c)", fontSize: 13.5, fontWeight: 500, fontFamily: "inherit", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #e8e2da", overflow: "hidden", boxShadow: "0 2px 20px rgba(0,0,0,0.06)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr 1fr auto", gap: 16, padding: "0.6rem 2rem", background: "#faf8f5", borderBottom: "1px solid #f0ebe4" }}>
              {["Order ID", "Items", "Total", "Status", ""].map((h) => (
                <div key={h} style={{ fontSize: 11, fontWeight: 500, color: "#9e9085", textTransform: "uppercase", letterSpacing: "0.06em" }}>{h}</div>
              ))}
            </div>
            ...
            {mockOrders.map((order, idx) => (
              <div
                key={order.id}
                style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr 1fr auto", alignItems: "center", gap: 16, padding: "1rem 2rem", borderBottom: idx < mockOrders.length - 1 ? "1px solid #f5f0eb" : "none", transition: "background 0.15s", cursor: "default" }}
              ...
              >
                ...
                <div>
                  <span style={{ ...statusColors[order.status], fontSize: 12, fontWeight: 500, padding: "3px 10px", borderRadius: 99, display: "inline-block" }}>
                    {order.status}
                  </span>
                </div>
                <button style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12.5, color: "#e74c3c", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position: "fixed", bottom: "1.5rem", left: "50%", transform: "translateX(-50%)", background: "#2b2420", color: "#fff", padding: "10px 20px", borderRadius: 10, fontSize: 13.5, fontFamily: "inherit", whiteSpace: "nowrap", zIndex: 999, animation: "fadeUp 0.2s ease" }}>
          {toast}
        </div>
      )}
