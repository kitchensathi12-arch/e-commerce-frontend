  ...
  >
    ...
    {brand.certificate && (
      <button
        type="button"
        onClick={(e) =>
          handleCertificateDownload(e, brand.certificate!, brand.brand_name)
        }
    ...
    {brand.brand_logo ? (
      ...
      <div className="h-20 md:h-24 flex items-center justify-center mb-6">
        <span className="text-3xl font-bold text-[#1a1208]/20 tracking-tighter select-none">
    ...
    {/* Brand Name */}
    <h3 className="font-medium text-[#1a1208] text-base md:text-lg tracking-tight text-center leading-tight">
      {brand.brand_name}
    </h3>
    ...
    <section className="w-screen relative left-1/2 -translate-x-1/2 bg-[#f8f6f2] py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col items-center text-center mb-12">
          <span className="uppercase tracking-[0.125em] text-xs font-medium text-[#1a1208]/60 mb-2">
            DISCOVER PREMIUM LABELS
          </span>
          <h2 className="text-4xl md:text-5xl font-semibold text-[#1a1208] tracking-tight">
            Shop by Brand
          </h2>
        ...
        {isLoading ? (
          <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="min-w-[220px] h-40 bg-white rounded-3xl shadow-sm animate-pulse"
              />
            ))}
          </div>
          ...
          >
            ...
            <div
              ref={sliderRef}
              className="flex w-max animate-fastScroll gap-6 md:gap-8 py-4"
            >
          ...
          ">
            {[
              ...
              <div key={label} className="flex flex-col items-center gap-1 sm:gap-1.5">
                <span className="text-xl sm:text-2xl lg:text-3xl font-bold text-[#1a1208] tracking-tight">
                  {value}
                </span>
                <span className="text-[10px] sm:text-xs lg:text-sm text-[#9a876a] uppercase tracking-widest font-medium">
                  {label}
                </span>
              </div>
            ))}
