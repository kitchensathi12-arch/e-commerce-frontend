...
const BrandProductsPage = () => {
  ...
  // ✅ find selected brand
  const selectedBrand = brands.find((b) => b.slug === slug);
  ...
  return (
    <section className="py-16 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center text-center mb-12">
            <div className="group relative bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg transition">
              ...
              {/* Logo */}
              {selectedBrand.brand_logo && <img src={selectedBrand.brand_logo} alt={selectedBrand.brand_name || 'brand'} className="h-20 object-contain mx-auto mb-4" />}

              {/* Name */}
              <h1 className="text-2xl md:text-3xl font-bold">{selectedBrand.brand_name}</h1>

              {/* Description */}
              {selectedBrand.description && <p className="text-gray-500 mt-2 max-w-xl">{selectedBrand.description}</p>}
            </div>
          </div>
        ) : (
        ...
        <div>
          <h2 className="text-xl md:text-2xl font-bold mb-6">Products</h2>
