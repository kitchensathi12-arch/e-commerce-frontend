  ...
  return (
    <>
      ...
      <div className="bg-black text-white text-center py-2.5 text-[13.5px] font-medium relative z-[101]">
        ...
        <a href="#" className="text-[#f4a261] underline">
          Shop Now
        </a>
      </div>
      ...
      <header className="w-full bg-white border-b border-gray-200 sticky top-0 z-50 font-sans">
        <div className="w-full px-4 md:px-8 lg:px-[60px] h-[68px] md:h-[78px] flex items-center justify-between">
          ...
          <nav className="hidden md:flex">
            {navLinks.map((l) => (
              <Link key={l.to} to={l.to} className="mx-5 font-medium text-[#333] text-sm no-underline hover:text-[#e63939] transition-colors">
                {l.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-3.5 md:gap-[18px]">
            ...
            <Link to="/cart" className="relative">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-[#e63939] text-white text-[10px] font-bold w-[18px] h-[18px] rounded-full flex items-center justify-center">{cartCount}</span>
              )}
            </Link>
            ...
            <div className="hidden md:flex items-center">
              {!user ? (
                <button onClick={() => navigate('/login')} className="flex items-center gap-2 px-5 py-2.5 bg-[#e63939] text-white rounded-full text-sm font-medium hover:bg-red-700 transition-colors">
                  <User size={16} /> Login
                </button>
              ) : (
                  ...
                  {profileOpen && (
                    <>
                      ...
                      <div className="absolute right-0 mt-3 w-52 bg-white rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.12)] py-2 border border-gray-100 z-[300]">
                        <Link to="/account" className="flex items-center gap-3 px-5 py-3 text-sm hover:bg-gray-50 text-gray-700 no-underline">
                          <User size={15} /> Profile
                        </Link>
                        <Link to="/orders" className="flex items-center gap-3 px-5 py-3 text-sm hover:bg-gray-50 text-gray-700 no-underline">
                          <ClipboardList size={15} /> Orders
                        </Link>
                        {user?.role === 'admin' && (
                          <Link to="/dashboard" className="flex items-center gap-3 px-5 py-3 text-sm hover:bg-gray-50 text-blue-600 font-medium no-underline">
                            Admin Panel
                          </Link>
                        )}
      ...
      {/* Mobile Overlay */}
      {mobileOpen && <div className="fixed inset-0 bg-black/30 z-[200] md:hidden" onClick={() => setMobileOpen(false)} />}
        ...
        <div className="fixed inset-0 w-screen max-w-full h-full bg-white z-[201] overflow-y-auto pt-[68px] md:hidden font-sans">
          <div className="px-5 py-5 space-y-5">
            ...
            {!user ? (
              ...
              <div>
                <Link to="/account" className="flex items-center gap-3 px-4 py-3 rounded-xl text-[#333] font-medium text-[15px] no-underline hover:bg-red-50 hover:text-[#e63939] transition-colors">
                  <User size={17} /> Profile
                </Link>
                <Link to="/orders" className="flex items-center gap-3 px-4 py-3 rounded-xl text-[#333] font-medium text-[15px] no-underline hover:bg-red-50 hover:text-[#e63939] transition-colors">
                  <ClipboardList size={17} /> Orders
                </Link>
                {user?.role === 'admin' && (
                  <Link to="/dashboard" className="flex items-center gap-3 px-4 py-3 rounded-xl text-blue-600 font-medium text-[15px] no-underline hover:bg-red-50 transition-colors">
                    Admin Panel
                  </Link>
                )}
                <button
