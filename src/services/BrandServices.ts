...
import type { IBrandPayload } from '@kitchensathi12-arch/ecommerce-types';
...
export const createBrand = async (payload: IBrandPayload): Promise<BrandResponse> => {
...
export const getAllBrands = async ({ page = 1, limit = 10 }: { page?: number; limit?: number }): Promise<BrandListResponse> => {
...
export const updateBrand = async ({ id, payload }: { id: string; payload: FormData }) => {
  const res = await API.put(`/brand/update-brand/brandId/${id}`, payload);
...
export const deleteBrand = async (id: string): Promise<BrandResponse> => {
  const res = await API.delete(`/brand/delete-brand/brandId/${id}`);
