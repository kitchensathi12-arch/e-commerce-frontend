...
import { Trash2, Package, Tags, FolderTree, RotateCcw, Search, Ribbon } from 'lucide-react';
import { getDeletedCategories, restoreCategory } from '@/services/CategoryServices';
...
const BinPage = () => {
  ...
  const { mutate: handleRestoreCategory, isPending: isRestoringCategory } = useMutation({
              ...
              {!productLoading &&
                ...
                filteredProducts.map((product) => (
                  <tr key={String(product._id)}>
                    ...
                    <td>{product.deleted_at ? new Date(product.deleted_at).toLocaleDateString('en-IN') : '—'}</td>
                    <td>
                      <div className="bin-actions">
                        <button className="btn btn-restore" disabled={isRestoringProduct} onClick={() => handleRestoreProduct(String(product._id))}>
                          <RotateCcw size={14} />
                          {isRestoringProduct ? 'Restoring...' : 'Restore'}
                        </button>
                        <button className="btn btn-delete" disabled={isDeletingProduct} onClick={() => confirmDeleteProduct(String(product._id))}>
                          <Trash2 size={14} />
                  ...
                  <tr key={String(brand._id)}>
                    <td>
                      <div className="brand-cell">
                        {(brand as any).brand_logo && <img src={(brand as any).brand_logo} alt={brand.brand_name} className="brand-logo" />}
                        {brand.brand_name ?? '—'}
                      </div>
                    </td>
                    <td>{(brand as any).deleted_at ? new Date((brand as any).deleted_at).toLocaleDateString('en-IN') : '—'}</td>
                    <td>
                      <div className="bin-actions">
                        <button className="btn btn-restore" disabled={isRestoringBrand} onClick={() => handleRestoreBrand(String(brand._id))}>
                          <RotateCcw size={14} />
              ...
              {!categoryLoading &&
                ...
                filteredCategories.map((category) => (
                  <tr key={String(category._id)}>
                    ...
                    <td>{category.deleted_at ? new Date(category.deleted_at).toLocaleDateString('en-IN') : '—'}</td>

                    <td>
                      <div className="bin-actions">
                        <button className="btn btn-restore" disabled={isRestoringCategory} onClick={() => handleRestoreCategory(String(category._id))}>
                          <RotateCcw size={14} />
