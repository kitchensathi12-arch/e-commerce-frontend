...
import { getAllCategories, updateCategory, deleteCategory } from '@/services/CategoryServices';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import type { ICategoryDocument, ICategoryListResponse } from '@kitchensathi12-arch/ecommerce-types';
import toast from 'react-hot-toast';

const CategoryPage = () => {
  const [openModal, setOpenModal] = useState(false);
  const [viewModalOpen, setViewModalOpen] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState<ICategoryDocument | undefined>();
  const [search, setSearch] = useState('');

  const queryClient = useQueryClient();

  // ================= FETCH =================
  const { data, isLoading } = useQuery<ICategoryListResponse>({
    ...
    <div className="flex flex-col gap-6 mb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Organize and control your product categories</p>
        </div>
          ...
          {filteredCategories.map((category) => (
            ...
            >
              <div className="flex flex-col min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-1 truncate">Slug: {category.slug}</p>

                <span className={`mt-4 inline-flex w-fit text-xs px-4 py-1 rounded-full ${category.isActive ? 'bg-green-100 text-green-600' : 'bg-gray-200 text-gray-500'}`}>
              ...
              >
                ...
                <Trash2 size={18} className="cursor-pointer text-red-500 hover:scale-110 transition" onClick={() => handleDelete(category._id.toString())} />
              </div>
            </div>
          ))}
        </div>
